...
public:
    mp3player_t();
    mp3player_t(LPCTSTR pszName, BOOL bFile = FALSE, BOOL bMp3 = FALSE);
    virtual ~mp3player_t();
...
inline mp3player_t::mp3player_t() : m_pvWave(NULL), m_cbWave(0), m_bAlloc(FALSE)
{
}

inline void mp3player_t::Unload()
{
    m_binWave.clear();
    m_pvWave = NULL;
    m_cbWave = 0;
...
inline /*static*/ void mp3player_t::StopAll(void)
{
...
retry:
    ...
    padding = (head[2] >> 1) & 1;
    cbFrame = anMp3SmpPerFrame_[ver][layer] / 8 * bitrate * 1000 / samp + padding;
...
inline BOOL ConvertMp3ToWaveDx(std::string& bin, LPBYTE& pb, DWORD& cb)
{
    ...
    if (::acmFormatSuggest(NULL, (WAVEFORMATEX *)&wfmp3, &wfwav, sizeof(WAVEFORMATEX), ACM_FORMATSUGGESTF_WFORMATTAG))
    {
        assert(0);
        return FALSE;
    }

    if (::acmStreamOpen(&has, NULL, (WAVEFORMATEX *)&wfmp3, &wfwav, NULL, 0, 0, ACM_STREAMOPENF_NONREALTIME))
    {
        return FALSE;
    }

    ::acmStreamSize(has, DWORD(wfmp3.nBlockSize + 1), &cbWavBuf, ACM_STREAMSIZEF_SOURCE);

    std::string mp3_buf, wav_buf;
    mp3_buf.resize(cbMp3Buf);
    wav_buf.resize(cbWavBuf);
    ...
    bin.append("RIFF????WAVEfmt ", 16);
    cbTemp = sizeof(WAVEFORMATEX);
    bin.append((PCHAR)&cbTemp, 4);
    bin.append((PCHAR)&wfwav, cbTemp);
    bin.append((PCHAR)"data????", 8);
    ...
    for(cbData = /*cFrames =*/ 0; ; /*cFrames++*/)
    {
        ...
        {
            bin.append((PCHAR)&wav_buf[0], ash.cbDstLengthUsed);
...
inline
mp3player_t::mp3player_t(LPCTSTR pszName, BOOL bFile/* = FALSE*/, BOOL bMp3/* = FALSE*/)
    : m_pvWave(NULL), m_cbWave(0), m_bAlloc(FALSE)
{
    Load(pszName, bFile, bMp3);
}

inline /*virtual*/ mp3player_t::~mp3player_t()
{
}

inline BOOL
mp3player_t::Load(LPCTSTR pszName, BOOL bFile/* = FALSE*/, BOOL bMP3/* = FALSE*/)
{
    if (bMP3)
    {
        if (bFile)
...
inline BOOL mp3player_t::LoadWaveFromFile(LPCTSTR pszFile)
{
    using namespace std;
    size_t cb;
    PBYTE pb = MFile_GetContents(pszFile, &cb);
    if (cb > 12 &&
        ...
        pb[8] == 'W' && pb[9] == 'A' && pb[10] == 'V' && pb[11] == 'E')
    {
        ...
        m_pvWave = NULL;
        m_cbWave = 0;
        m_binWave.assign((const char *)pb, (const char *)pb + cb);
        free(pb);
        return TRUE;
    }
    free(pb);
...
inline BOOL
mp3player_t::LoadWaveFromResource(LPCTSTR pszResource, HINSTANCE hInstance/* = NULL*/)
{
    if (hInstance == NULL)
...
inline BOOL
mp3player_t::LoadWaveFromResource(UINT nResourceID, HINSTANCE hInstance/* = NULL*/)
{
    return LoadWaveFromResource(MAKEINTRESOURCE(nResourceID), hInstance);
}

inline BOOL mp3player_t::LoadMp3FromFile(LPCTSTR pszFile)
{
    using namespace std;
    size_t cb;
    BOOL bOK = FALSE;
...
inline BOOL
mp3player_t::LoadMp3FromResource(LPCTSTR pszResource, HINSTANCE hInstance/* = NULL*/)
{
    if (hInstance == NULL)
        hInstance = ::GetModuleHandle(NULL);
...
inline BOOL
mp3player_t::LoadMp3FromResource(UINT nResourceID, HINSTANCE hInstance/* = NULL*/)
{
    return LoadMp3FromResource(MAKEINTRESOURCE(nResourceID), hInstance);
}

inline BOOL mp3player_t::LoadMp3FromMemory(LPVOID pv, DWORD cb)
{
    LPBYTE pb = (LPBYTE)pv;
...
inline BOOL mp3player_t::Play(BOOL bRepeat/* = FALSE*/)
{
    if (m_bAlloc)
    {
        if (bRepeat)
            return ::PlaySound((LPTSTR)&m_binWave[0], NULL, SND_LOOP | SND_ASYNC | SND_MEMORY | SND_NODEFAULT | SND_NOSTOP);
        else
            return ::PlaySound((LPTSTR)&m_binWave[0], NULL, SND_ASYNC | SND_MEMORY | SND_NODEFAULT | SND_NOSTOP);
    }
    else
    {
        if (bRepeat)
            return ::PlaySound((LPCTSTR)m_pvWave, NULL, SND_LOOP | SND_ASYNC | SND_MEMORY | SND_NODEFAULT | SND_NOSTOP);
        else
            return ::PlaySound((LPCTSTR)m_pvWave, NULL, SND_ASYNC | SND_MEMORY | SND_NODEFAULT | SND_NOSTOP);
    }
}

inline LPVOID mp3player_t::GetPtr()
{
    if (m_bAlloc)
        return &m_binWave[0];
    else
...
inline BOOL mp3player_t::PlayAndWait()
{
    return ::PlaySound((LPCTSTR)GetPtr(), NULL, SND_SYNC | SND_MEMORY | SND_NODEFAULT | SND_NOSTOP);
}

inline BOOL mp3player_t::SaveToWaveFile(LPCTSTR pszFile)
{
    BOOL b = MFile_PutContents(pszFile, &m_binWave[0], m_binWave.size());
    return b;
}

inline void mp3player_t::Stop()
{
